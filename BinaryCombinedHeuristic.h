...
void BinaryCombinedHeuristic_AStar(Graph * graph, Graph* reverse_graph, int source, int destination, double W, int TimeLimit, Results * Risultati){
    ...
    while( (check1 == 0)  &&  ((double)(clock() - startHeur ) / CLOCKS_PER_SEC) < TimeLimit){
        ...
        if( LP_length <= W){ //Check on feasibility
            if( LP_sunLength < bestSolSunLength || ( LP_sunLength == bestSolSunLength && LP_length < bestSolLength)){ //Se la soluzione è migliorativa
                ...
                if (Risultati->numImprovements == Risultati->capImprovements) {
                    ...
                    Improvement *newArr = (Improvement*)realloc(Risultati->improvements, (size_t)newCap * sizeof(Improvement));
                    if (newArr != NULL) {
        ...
        }else{ //If the path is not feasible, a binary search for the best value of lambda starts
    ...
    Risultati->SP_Length = SP_length; //calcolo sp1 prendo distanza (min distanza)
    Risultati->SP_SunLength = SP_sunLength; //calcolo sp1 prendo risorsa (min distanza)
