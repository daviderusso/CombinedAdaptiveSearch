...
// Versione senza malloc/free: scrive il path in outPath (buffer preallocato)
// Ritorna la lunghezza del path; se non esiste path, ritorna 0.
static inline int reconstructPathInto(const int *pred, int dest, int *outPath, int maxNodes) {
    int count = 0;
    int current = dest;

    while (current != -1) {
        if (count >= maxNodes) return 0; // overflow buffer
        outPath[count++] = current;
        current = pred[current];
    }
