...
// --- Buffer riusati per tutte le iterazioni BCH (NO malloc/free nei cicli) ---
...
int    *LP_path               = (int *) malloc(graph->numNodes * sizeof(int));
int     lambdaPathLength      = 0;
if (!distFromSource_Lambda || !predFromSource_Lambda || !LP_path) {
    fprintf(stderr, "Errore allocazione buffer BCH.\n");
    exit(EXIT_FAILURE);
}
    ...
    while ((check1 == 0) && ((double) (clock() - begin_heur) / CLOCKS_PER_SEC) < TimeLimit) {
        ...
        if (check) {
            ...
            check = false;
        }        lambdaPathLength = reconstructPathInto(predFromSource_Lambda, destination, LP_path, graph->numNodes);
        double SP_C1_Length = computePathCost1(g_red, LP_path, lambdaPathLength);
        double SP_C2_length = computePathCost2(g_red, LP_path, lambdaPathLength);        if (SP_C2_length <= W) {
            ...
            while ((curr_iter < maxIter) && ((double) (clock() - begin_heur) / CLOCKS_PER_SEC < TimeLimit)) {
                ...
                astar_lambda(g_red, source, destination, distFromSource_Lambda, predFromSource_Lambda, currLambda,
                             distToDest);                lambdaPathLength = reconstructPathInto(predFromSource_Lambda, destination, LP_path, graph->numNodes);
                SP_C1_Length = computePathCost1(g_red, LP_path, lambdaPathLength);
                SP_C2_length = computePathCost2(g_red, LP_path, lambdaPathLength);                if (SP_C2_length <= W) {
                    // Check on feasibility
                    if (SP_C1_Length < bestSol_C1_Length || (
                            SP_C1_Length == bestSol_C1_Length && SP_C2_length < bestSol_C2_Length)) {
                        //If the path has a lower objective function value than the best solution or it consumes less resources, then
