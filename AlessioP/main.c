...
static void print_usage(const char *prog) {
    fprintf(stderr,
            "Uso: %s [--graph FILE] [--instances FILE] [--res-dir DIR] [--timelimit SECONDS]\n",
            prog);
}
...
static int parse_args(int argc, char **argv, Config *cfg) {
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--graph") == 0) {
            ...
            cfg->graph_file = argv[++i];
        } else if (strcmp(argv[i], "--instances") == 0) {
            if (i + 1 >= argc) return -1;
            cfg->instances_file = argv[++i];
        } else if (strcmp(argv[i], "--res-dir") == 0) {
            if (i + 1 >= argc) return -1;
            cfg->res_dir = argv[++i];
        } else if (strcmp(argv[i], "--timelimit") == 0) {
            if (i + 1 >= argc) return -1;
            cfg->time_limit = atoi(argv[++i]);
            if (cfg->time_limit <= 0) return -1;
...
int main(int argc, char **argv) {
    ...
    if (parse_rc != 0) {
        ...
        return parse_rc < 0 ? EXIT_FAILURE : EXIT_SUCCESS;
    }

    const char *filename = cfg.graph_file; // File name of the graph
    ...
    const char *filein = cfg.instances_file;
    int TimeLimit = cfg.time_limit;

    const char *res_dir = cfg.res_dir;
